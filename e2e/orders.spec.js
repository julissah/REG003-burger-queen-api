    ...
    Promise.all([
      ...
      .then(([product, user]) => (
        Promise.all([
          fetchAsTestUser('/orders', {
            method: 'POST',
            body: { products: [{ productId: product._id, qty: 50 }], userId: user._id, client: user._id },
          }),
          fetchAsAdmin('/orders', {
            method: 'POST',
            body: { products: [{ productId: product._id, qty: 25 }], userId: user._id, client: user._id },
          }),
        ])
          .then((responses) => {
            expect(responses[0].status).toBe(200);
...
describe('GET /orders/:orderId', () => {
  ...
  it('should get order as user', () => (
    Promise.all([
      ...
      .then(([product, user]) => fetchAsTestUser('/orders', {
        method: 'POST',
        body: { products: [{ productId: product._id, qty: 5 }], userId: user._id, client: user._id },
      }))
      .then((resp) => {
        expect(resp.status).toBe(200);
        return resp.json();
